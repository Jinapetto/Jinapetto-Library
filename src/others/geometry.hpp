...
// 線分 ab に対する点 c の位置: +1反時計, -1時計, +2後方, -2前方, 0線分上
...
// 円と円の関係: 4=離れている, 3=外接, 2=交わる, 1=内接, 0=包含
...
Real area(const Polygon &p, const Circle &c) {
    ...
    function<Real(Circle, Point, Point)> cross_area = [&](const Circle &c, const Point &a, const Point &b) {
        ...
        for(int i = 0; i + 1 < (int)tot.size(); i++) ret += cross_area(c, tot[i], tot[i + 1]);
        return ret;
    };
    Real A = 0;
    for(int i = 0; i < (int)p.size(); i++) A += cross_area(c, p[i], p[(i + 1) % p.size()]);
    return A;
}

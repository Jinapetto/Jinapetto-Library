//problem https://atcoder.jp/contests/abc289/tasks/abc289_g
#include<bits/stdc++.h>
using namespace std;
...
template<bool isMin = true> struct CHT {
	...
	using D = long double;
	inline bool check(const pair<ll,ll>& a, const pair<ll,ll>& b, const pair<ll,ll>&c){
	...
	void add(ll a, ll b) {
		...
		if(empty()) v.emplace_front(line);
			...
			if(ll c = v[0].x; c <= a) {
				...
				while(v.size() >= 2 and check(line, v[0], v[1])) v.pop_front();
				...
				while(v.size() >= 2 and check(v[v.size() - 2], v.back(),line)) v.pop_back();
				v.emplace_back(line);
			}
		}
	}
	ll get_y(const pair<ll,ll>& a, const ll& x) { return a.x * x +a.y;}
	ll query(ll x) {
		assert(!empty());
		int l = -1, r = v.size() - 1;
		while(l + 1 < r) {
			int m = (l + r) >> 1;
			if(get_y(v[m], x) >= get_y(v[m + 1], x)) l = m;else r = m;
		}
		return get_y(v[r], x) * (isMin ? 1 : -1);
	}
	ll query_monotone_inc(ll x) {
		assert(!empty());
		while(v.size() >= 2 and get_y(v[0], x) >= get_y(v[1],x))v.pop_front();
		return get_y(v[0], x) * (isMin ? 1 : -1);
	}
	ll query_monotone_dec(ll x) {
		assert(!empty());
		while(v.size() >= 2 and get_y(v.back(), x) >= get_y(v.end()[-2], x)) v.pop_back();
